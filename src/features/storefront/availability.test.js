...
describe('regla de compra única', () => {
  it('sin precio conocido no se añade en ningún modo', () => {
    for (const mode of ['demo', 'shopify']) {
      expect(
        canAddProduct({ ...product, price: '??' }, { ...variant, price: null }, { mode })
      ).toBe(false);
      expect(canAddProduct({ ...product, price: null }, null, { mode })).toBe(false);
    }
    expect(purchaseState({ ...product, price: null }, null, { mode: 'demo' })).toBe(
      PURCHASE_STATES.NOTIFY
    );
  });
  ...
  it('los conceptos siempre son vista previa', () => {
    expect(
      purchaseState({ ...product, isPreview: true }, variant, {
        mode: 'shopify',
        cartEnabled: true,
      })
    ).toBe(PURCHASE_STATES.PREVIEW);
  ...
  it('en Shopify exige carrito activo y stock', () => {
    ...
    expect(
      purchaseState(
        ...
        { mode: 'shopify', cartEnabled: true }
      )
    ).toBe(PURCHASE_STATES.SOLD_OUT);
