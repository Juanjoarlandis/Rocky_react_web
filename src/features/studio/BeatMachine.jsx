...
const STORAGE_KEY = STORAGE_KEYS.beatTable;
...
function BeatMachine() {
  ...
  const share = async () => {
    ...
    try {
      ...
      copiedRef.current = setTimeout(() => setCopied(false), 2400);
    } catch {
  ...
  return (
    <div className="mesa-wrap" ref={wrapRef}>
      <img
        ...
        height="600"
        alt=""
        className="doodle mesa-dancer neon-art"
        style={danceStyle}
      />
      ...
      <div className="mesa">
        <header className="mesa-cabecera">
          ...
          <div className="mesa-lcd" aria-hidden="true">
            <div className="lcd-datos">
              ...
              <span>
                <b>{Math.round(swing * 100)}</b>% swing
              </span>
              <span>
                <b>{tonality.label}</b>
              </span>
        ...
        <div className="mesa-transporte">
          ...
          <button
            type="button"
            className={`mesa-rec ${recording ? 'is-on' : ''}`}
        ...
        <div className="mesa-presets" role="group" aria-label="Ritmos de fábrica">
          ...
          {PRESETS.map((preset) => (
            <button
              key={preset.id}
              type="button"
              className={`chip ${activePreset === preset.id ? 'is-on' : ''}`}
        ...
        <div className="mesa-seq">
          <div className="seq-scroll">
            ...
            <div className="seq-rejilla" role="group" aria-label="Secuenciador de 16 pasos">
              {TRACKS.map((track, index) => (
                <fieldset className={`seq-fila ${audible(index) ? '' : 'apagada'}`} key={track.id}>
                  <legend className="sr-only">{track.label}</legend>
                  <div className="seq-cabeza">
                    <button
                      type="button"
                      className={`pista-nombre ${mutes[index] ? 'muda' : ''}`}
                      onClick={() => setMutes((prev) => prev.map((m, i) => (i === index ? !m : m)))}
                    ...
                    <button
                      type="button"
                      className={`pista-solo ${solos[index] ? 'is-on' : ''}`}
                      onClick={() => setSolos((prev) => prev.map((s, i) => (i === index ? !s : s)))}
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setLevels((prev) =>
                          prev.map((n, i) => (i === index ? Number(e.target.value) : n))
                        )
                      }
          ...
          <div className="pads-barra">
            <div className="pads-bancos" role="group" aria-label="Banco de sonidos">
              {BANCOS.map((b) => (
                <button
                  key={b.id}
                  type="button"
                  className={`chip ${bank === b.id ? 'is-on' : ''}`}
            ...
            <label className="pads-tono">
              ...
              <select
                value={tonality.id}
                onChange={(e) =>
                  setTonality(TONALIDADES.find((t) => t.id === e.target.value) || TONALIDADES[0])
                }
              >
                {TONALIDADES.map((t) => (
        ...
        <footer className="mesa-pie">
          ...
          <p className="mesa-nota">
            * Sonidos cocinados en el navegador, sin un solo sample. Comparte el enlace y tu beat se
            abre sonando.
          </p>
        </footer>
      </div>
