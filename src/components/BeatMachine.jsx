...
function BeatMachine() {
  ...
  const tocarCelda = (indice, paso, velocidad) => {
    ...
    const siguiente =
      velocidad !== undefined ? velocidad : CICLO[(CICLO.indexOf(actual) + 1) % CICLO.length];
    setPattern((prev) =>
      prev.map((track, i) =>
  ...
  const tap = () => {
    ...
    if (tapsRef.current.length >= 2) {
      const huecos = tapsRef.current.slice(1).map((t, i) => t - tapsRef.current[i]);
          ...
          <div className="seq-scroll">
            ...
            <div className="seq-rejilla" role="group" aria-label="Secuenciador de 16 pasos">
              {TRACKS.map((track, indice) => (
                <fieldset className={`seq-fila ${audible(indice) ? '' : 'apagada'}`} key={track.id}>
                  ...
                  <div className="seq-cabeza">
                    <button
                      ...
                      onClick={() =>
                        setMudas((prev) => prev.map((m, i) => (i === indice ? !m : m)))
                      }
                    ...
                    <button
                      ...
                      onClick={() =>
                        setSolos((prev) => prev.map((s, i) => (i === indice ? !s : s)))
                      }
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setNiveles((prev) =>
                          prev.map((n, i) => (i === indice ? Number(e.target.value) : n))
                        )
                      }
          ...
          <div className="pads-barra">
            ...
            <label className="kicker pads-tono">
              ...
              <select
                value={tonalidad.id}
                onChange={(e) =>
                  setTonalidad(TONALIDADES.find((t) => t.id === e.target.value) || TONALIDADES[0])
                }
              >
                {TONALIDADES.map((t) => (
