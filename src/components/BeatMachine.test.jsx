...
describe('La mesa de beats', () => {
    ...
    it('dibuja las ocho pistas con sus dieciséis pasos', () => {
        ...
        expect(filas).toHaveLength(TRACKS.length);
        expect(within(filas[0]).getAllByRole('gridcell')).toHaveLength(STEP_COUNT);
    });

    it('arranca con el ritmo de fábrica cargado', () => {
        montar();
        const bombo = within(screen.getAllByRole('row')[0]).getAllByRole('gridcell');
    ...
    it('al pinchar una celda va rotando golpe, acento y fantasma', async () => {
        ...
        montar();
        const palmas = within(screen.getAllByRole('row')[4]).getAllByRole('gridcell');
    ...
    it('carga los ritmos de fábrica con su tempo', async () => {
        ...
        const caja = within(screen.getAllByRole('row')[1]).getAllByRole('gridcell');
    ...
    it('limpia la rejilla entera', async () => {
        ...
        screen.getAllByRole('gridcell').forEach((celda) => {
    ...
    it('silencia y pone en solo cada pista', async () => {
        const user = userEvent.setup();
        montar();
        const cabeza = screen.getAllByRole('row')[0];
    ...
    it('comparte un enlace con el beat, el tempo y el swing', async () => {
        ...
        Object.defineProperty(navigator, 'clipboard', {
            configurable: true,
            value: { writeText: (texto) => { escrito.push(texto); return Promise.resolve(); } },
        });
