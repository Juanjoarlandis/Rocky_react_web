...
export function summarizeDrops(products) {
    ...
    products.forEach((product) => {
        ...
        if (!handle) return;
        if (!drops.has(handle)) {
            drops.set(handle, { handle, title: product.drop, total: 0, locked: 0, previews: 0, available: 0 });
        }
    ...
    return (
        <div className="page-container drops">
            ...
            <div className="doodle-shelf drops-list-wrap">
                ...
                <img src={tumbado} width="800" height="500" alt="" className="doodle drops-illustration neon-art al-ritmo al-ritmo--suave" style={{ '--fase': '0.35' }} />
                {/* El Dado se juega el próximo drop en la otra punta de la repisa */}
                <img src={dadoSentado} width="600" height="600" alt="" className="doodle drops-dado neon-art al-ritmo" style={{ '--fase': '0.55' }} />
                {/* Y el skater vuela por encima */}
                <img src={skaterOllie} width="600" height="600" alt="" className="doodle drops-skater neon-art al-ritmo" style={{ '--fase': '1.1' }} />
                <nav className="drops-list" aria-label="Categorías">
                    ...
                    {drops.map((drop) => (
                        ...
                        >
                            ...
                            <span className={`sticker drops-link-state drops-link-state--${drop.state === 'Disponible' ? 'available' : 'pending'}`}>
                                {drop.state}
                            </span>
                            <span className="drops-arrow" aria-hidden="true">→</span>
                        </Link>
