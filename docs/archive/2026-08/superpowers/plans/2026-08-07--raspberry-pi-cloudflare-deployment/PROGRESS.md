...
- Docker image: `rocky035:<tag-imagen>`
...
- Cloudflare tunnel: `<tunel-cloudflare>` (`<id-tunel>`), remotely managed configuration version 5.
...
- DNS and TLS: proxied CNAME records for apex and `www` target the tunnel subdomain; Cloudflare Universal SSL is active for `rocky035.com` and `*.rocky035.com`, and TLS validation succeeded. The old `<hostname-anterior>` ingress and DNS record were removed after the new hostnames passed verification.
...
- Existing services: `cloudflared`, `<otro-servicio>-tunnel`, and `<otro-servicio>` remained active; `https://<host-interno-crm>` continued returning 200; the pre-existing port `3001` listener was unchanged.
...
1. Recreate a proxied `CNAME` for `<hostname-anterior>` pointing to `<id-tunel>.cfargotunnel.com`.
2. Restore the tunnel ingress sequence to `<host-interno-crm>`, `<hostname-anterior>`, and the `http_status:404` catch-all.
...
1. In Cloudflare, remove only the `rocky035.com` and `www.rocky035.com` published-application routes and their two DNS records from `<tunel-cloudflare>`. Do not rotate the tunnel token or change `<host-interno-crm>`.
...
- The public route shares the healthy `<tunel-cloudflare>` connector. A tunnel outage or token rotation affects both hostnames even though their origin services remain separate.
